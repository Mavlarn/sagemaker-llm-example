...
def get_answer(query, vs_path, history, mode):
    if mode == "知识库问答":
        ...
        for resp, history in local_doc_qa.get_knowledge_based_answer(query=query, chat_history=history):
            source = "\n\n"
            source += "".join(
        ...
        for resp, history in local_doc_qa.get_direct_answer(query, history):
            history[-1][-1] = resp
            yield history, ""
...
def init_model():
    try:
        ...
        result = local_doc_qa.get_direct_answer("你好")
        # for resp, history in local_doc_qa.get_direct_answer(query, history):
        print(result["result"])
